...
if(!is_numeric($pid) || !is_numeric($report_id) || $report_id<1) {
	...
	if(!$module->isEnabledProject($pid)) {
		...
		if (!preg_match("/^[A-F|0-9]{32}$/", $token)) {
			...
			//print("output: " . $output->project_id);
/*			echo "<pre>";
			var_dump($output);
			echo "</pre>";
*/
			...
			if(!isset($output->project_id)  ) {
				...
				if($output->project_id != $pid ) {
					...
					if (!$user_has_access_right==true) {
						...
						// Executes the R script with the parameters provided by REDCap
						/*				
						exec('Rscript -e ".libPaths(c(/'C:/Users/RGamero_a/Documents/R/win-library/3.6/', /'C:/Program Files/R/R-3.6.1/library/')); Sys.setenv(RSTUDIO_PANDOC=/'C:/Program Files/RStudio/bin/pandoc/'); rmarkdown::render(/'C:/inetpub/wwwroot/network/redcap/plugins/graphs/R_Tables_and_Plots.Rmd/', params = list(pid = "'.$query['pid'].'", reportId = "'.$query['report_id'].'"))" 2>&1', $output);
						*/
						
						//harcoded parameters
						//$r_path = 'C:/Program Files/R/R-3.5.1/bin/Rscript.exe';
						//$pandocPath = "Sys.setenv(RSTUDIO_PANDOC='C:/Program Files/RStudio/bin/pandoc');"; 		
						//$arr_libPaths = Array("C:/Users/Alvaro/Documents/R/win-library/3.5","C:/Program Files/R/R-3.5.1/library");
